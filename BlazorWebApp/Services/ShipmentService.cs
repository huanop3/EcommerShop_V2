...
namespace BlazorWebApp.Services
{
    public class ShipmentService
    {
        ...
        private const string BaseUrl = "http://localhost:5282/main/api/Shipment";
        ...
        /// </summary>
        public async Task<List<AssignedOrderVM>> GetMyAssignedOrdersAsync()
        {
            await SetAuthorizationHeader();
            try
            {
                var response = await _httpClient.GetAsync($"{BaseUrl}/my-orders");
        ...
        /// <summary>
        /// Assign shipment cho shipper - Admin only
        /// </summary>
        public async Task<bool> AssignShipmentAsync(AssignShipmentRequest request)
        {
            ...
            try
            {
                ...
                var response = await _httpClient.PostAsync($"{BaseUrl}/assign", content);
        ...
        /// </summary>
        public async Task<ShipmentValidationResponse> CanOrderBeShippedAsync(int orderId)
        {
            if (orderId <= 0) 
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        return new ShipmentValidationResponse { IsValid = true, Message = "Order can be shipped" };
                    }
                }
